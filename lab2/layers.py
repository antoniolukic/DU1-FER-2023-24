...
class FC(Layer):
  ...
  def forward(self, inputs):
    ...
    # TODO
    self.inputs = inputs
    return self.inputs @ self.weights.T + self.bias
...
class ReLU(Layer):
  ...
  def backward_inputs(self, grads):
    ...
    """
    # TODO
    return grads * np.where(self.inputs > 0, 1, 0)
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    self.logits = exp_scores / sum_exp
    self.N = x.shape[0]
    return -np.sum(y * np.log(self.logits)) / self.N
